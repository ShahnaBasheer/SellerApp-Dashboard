...
@Component({
  ...
  imports: [ CardHeaderWrapperComponent, BaseChartDirective ],
...
export class SalesByRegionComponent {
  ...
  private salesData = [30000, 25000, 15000, 22000, 18000, 14000];
  ...
  private radarChartPlugins =
    {
      ...
      beforeDraw: (chart: any) => {
        ...
        for (let i = 0; i < steps; i++) {
          ...
          const startRadius = rScale.getDistanceFromCenterForValue(rScale.ticks[i].value);
          const endRadius = i < steps - 1 ? rScale.getDistanceFromCenterForValue(rScale.ticks[i + 1].value) : 0;
  ...
  ngAfterViewInit(): void {
    ...
    if (ctx) {
      this.salesByRegionChart = new Chart(ctx, {
        ...
        plugins: [ this.radarChartPlugins ]
      });
    }
